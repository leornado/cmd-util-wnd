 ...
 *   })
 */
function modify(ast, options, cfgs) {
  ast = getAst(ast);
  ...
  if (requirefn || asyncfn) {
    ast = replaceRequire(ast, cfgs, options, requirefn, asyncfn);
  }
  return ast;
}
 ...
 *   })
 */
function replaceRequire(ast, cfgs, options, requirefn, asyncfn) {
  ast = getAst(ast);
  ...
  var trans = new UglifyJS.TreeTransformer(function(node, descend) {
    ...
    if (node instanceof UglifyJS.AST_Call && cfgs && cfgs.customFns) {
      for (var i = 0; i < cfgs.customFns.length; i++) {
        var cr = cfgs.customFns[i], isRegExp = cr instanceof RegExp;
        if (options[cr] && (isRegExp && cr.test(node.expression.name) || !isRegExp && node.expression.name === cr) && node.args.length) {
          return replaceChild(node, options[cr]);
        }
      }
    }

    if (node instanceof UglifyJS.AST_ObjectKeyVal && cfgs && cfgs.customProps) {
      for (var i = 0; i < cfgs.customProps.length; i++) {
        var cp = cfgs.customProps[i], isRegExp = cp instanceof RegExp;
        if (options[cp] && node.key !== undefined &&
            (isRegExp && cp.test(node.key) || !isRegExp && node.key === cp) && node.value !== undefined) {
          node.value.value = options[cp](node.end.value);
          return;
        }
      }
    }

    if (node instanceof UglifyJS.AST_ObjectKeyVal && cfgs && cfgs.customKeys) {
      for (var i = 0; i < cfgs.customKeys.length; i++) {
        var ck = cfgs.customKeys[i], isRegExp = ck instanceof RegExp;
        if (options[ck] && node.key !== undefined &&
            (isRegExp && ck.test(node.key) || !isRegExp && node.key === ck)) {
          node.key.value = options[ck](node.end.value);
          return;
        }
      }
    }
